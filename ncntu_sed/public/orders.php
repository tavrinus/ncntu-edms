...
$canCreateOrders = $isDirector || $isDeputyDirector;
if ($action === 'create' && !$canCreateOrders) {
    $_SESSION['dashboard_error'] = 'У вас немає прав для створення наказів. Тільки директор або заступник директора можуть створювати накази.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (!empty($_FILES['document_file']['name'])) {
            ...
            if (empty($errors)) {
                $new_file_name = uniqid('order_') . '.' . $file_ext;
        ...
        if (empty($errors)) {
            ...
            if ($action === 'create') {
                $result = $documentController->createOrderDocument($data);
                if ($result) {
                    $_SESSION['dashboard_success'] = 'Наказ успішно створено';
                    redirect('dashboard.php?tab=orders');
                    exit;
                } else {
                    $error_message = 'Помилка при створенні наказу';
                }
                ...
                if ($result) {
                    $_SESSION['dashboard_success'] = 'Наказ успішно оновлено';
                    redirect('dashboard.php?tab=orders');
                    exit;
                } else {
                    $error_message = 'Помилка при оновленні наказу';
                    error_log("Error updating order #{$document_id}");
                }
            }
        } else {
...
if ($action === 'delete' && $document_id > 0) {
    if (($isDirector || $isDeputyDirector) && $documentController->canEditDocument($user_id, $document_id)) {
        ...
        if ($attachedLetters && !isset($_GET['confirmed'])) {
            ...
            $page_title = 'Підтвердження видалення наказу';
            ...
            <div class="container mt-5">
                <div class="card border-danger">
                    ...
                    <div class="card-body">
                        <h5 class="card-title">Наказ прикріплений до листів</h5>
                        <p class="card-text">
                            Наказ "<strong><?= htmlspecialchars($document['title']) ?></strong>" прикріплений до 
                            <?= count($attachedLetters) > 1 ? count($attachedLetters) . ' листів' : '1 листа' ?>.
                        </p>
                        <p class="card-text">
                            Якщо ви видалите цей наказ, він буде видалений з усіх листів, де він прикріплений.
                        </p>
                        <div class="d-flex justify-content-between mt-4">
                            <a href="<?= BASE_URL ?>dashboard.php?tab=orders" class="btn btn-secondary">
                                <i class="bi bi-arrow-left me-2"></i> Скасувати
                            </a>
                            <a href="<?= BASE_URL ?>orders.php?action=delete&id=<?= $document_id ?>&confirmed=1" 
                               class="btn btn-danger" 
                               onclick="return confirm('Ви впевнені, що хочете видалити цей наказ? Наказ буде видалений з усіх листів, де він прикріплений. Ця дія незворотна.')">
        ...
        if ($documentController->documentModel->deleteDocument($document_id)) {
            ...
            $_SESSION['dashboard_success'] = 'Наказ успішно видалено';
        } else {
            $_SESSION['dashboard_error'] = 'Помилка при видаленні наказу';
        }
    } else {
        $_SESSION['dashboard_error'] = 'У вас немає прав для видалення цього наказу';
    }
    redirect('dashboard.php?tab=orders');
    exit;
    ...
    if (($isDirector || $isDeputyDirector) && $documentController->canEditDocument($user_id, $document_id)) {
        if ($documentController->archiveDocument($document_id)) {
            $_SESSION['dashboard_success'] = 'Наказ успішно переміщено в архів';
        } else {
            $_SESSION['dashboard_error'] = 'Помилка при архівації наказу';
        }
    } else {
        $_SESSION['dashboard_error'] = 'У вас немає прав для архівації цього наказу';
    }
    redirect('dashboard.php?tab=orders');
    exit;
    ...
    if (($isDirector || $isDeputyDirector) && $documentController->canEditDocument($user_id, $document_id)) {
        if ($documentController->unarchiveDocument($document_id)) {
            $_SESSION['success_message'] = 'Наказ успішно відновлено з архіву';
        } else {
            $_SESSION['error_message'] = 'Помилка при відновленні наказу з архіву';
        }
    } else {
        $_SESSION['error_message'] = 'У вас немає прав для відновлення цього наказу з архіву';
    }
...
if ($action === 'edit' && $document_id > 0) {
    $document = $documentController->documentModel->getDocumentById($document_id);
    if (!$document || $document['type_id'] != 2) {
        redirect('dashboard.php?tab=orders');
        exit;
    }
    if (!$isDirector && !$isDeputyDirector) {
...
if ($action === 'create') {
    $page_title = 'Створення наказу';
} elseif ($action === 'edit') {
    $page_title = 'Редагування наказу';
} elseif ($action === 'view') {
    $page_title = 'Перегляд наказу';
} else {
    $page_title = 'Накази';
}
require_once '../app/views/includes/header.php';
...
<!-- Додаткові стилі для сторінки наказів -->
<style>
.document-item {
    ...
    <div class="row g-0">
        ...
        <div class="col-md-3 user-sidebar sidebar" id="userSidebar">
            ...
            <div class="list-group rounded-0">
                ...
                <a href="<?= BASE_URL ?>dashboard.php?tab=orders" class="list-group-item list-group-item-action active">
            ...
            <div class="card border-dark shadow-sm mb-4">
                <div class="card-header bg-white">
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <?php else: ?>
                            <a href="<?= BASE_URL ?>dashboard.php?tab=orders" class="btn btn-outline-secondary me-2">
                ...
                <div class="card-body">
                    <?php if ($action === 'create' || $action === 'edit'): ?>
                        ...
                        <form action="<?= BASE_URL ?>orders.php?action=<?= $action ?><?= $document_id ? '&id=' . $document_id : '' ?>" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                            <div class="mb-3">
                                <label for="title" class="form-label">Назва наказу <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="title" name="title" required 
                                       value="<?= isset($document) ? htmlspecialchars($document['title']) : '' ?>">
                            </div>
                            <div class="mb-3">
                                <label for="content" class="form-label">Зміст наказу <span class="text-danger">*</span></label>
                            ...
                            <div class="mb-3">
                                <label for="document_file" class="form-label">Файл наказу</label>
                            ...
                            <div class="mt-4 d-flex justify-content-between">
                                ...
                                <?php else: ?>
                                <a href="<?= BASE_URL ?>dashboard.php?tab=orders" class="btn btn-outline-secondary">
                                    <i class="bi bi-arrow-left me-1"></i> Скасувати
                                </a>
                                <?php endif; ?>
                                <button type="submit" class="btn btn-success">
                                    <i class="bi bi-save me-1"></i> <?= $action === 'create' ? 'Створити наказ' : 'Зберегти зміни' ?>
                                </button>
                            </div>
                        </form>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                    ...
                                    <ul class="list-group list-group-flush">
                                        ...
                                        <?php if ($document['is_archived']): ?>
                                            <li class="list-group-item p-2 text-danger">
                                                <strong><i class="bi bi-archive me-1"></i> Наказ в архіві</strong>
                                            </li>
                                        <?php endif; ?>
                                    </ul>
                                </div>
                                ...
                                <?php if (($document['author_id'] == $user_id) && ($isDirector || $isDeputyDirector)): ?>
                                    <div class="d-grid gap-2">
                                        <?php if (!$document['is_archived']): ?>
                                        <a href="<?= BASE_URL ?>orders.php?action=edit&id=<?= $document_id ?>" class="btn btn-primary">
                                            <i class="bi bi-pencil me-1"></i> Редагувати
                                        </a>
                                        <?php endif; ?>
                                        <?php if ($document['is_archived']): ?>
                                            <a href="<?= BASE_URL ?>orders.php?action=unarchive&id=<?= $document_id ?>&from_archive=1" class="btn btn-success" 
                                               onclick="return confirm('Ви впевнені, що хочете відновити цей наказ з архіву?')">
                                                <i class="bi bi-archive-fill me-1"></i> Відновити з архіву
                                            </a>
                                        <?php else: ?>
                                            <a href="<?= BASE_URL ?>orders.php?action=archive&id=<?= $document_id ?>" class="btn btn-warning" 
                                               onclick="return confirm('Ви впевнені, що хочете перемістити цей наказ в архів?')">
                                                <i class="bi bi-archive me-1"></i> В архів
                                            </a>
                                        <?php endif; ?>
                                        <a href="<?= BASE_URL ?>orders.php?action=delete&id=<?= $document_id ?>" class="btn btn-danger" 
                                           onclick="return confirm('Ви впевнені, що хочете видалити цей наказ? Ця дія незворотна.')">
                    ...
                    <?php else: ?>
                        ...
                        ?>
                    <div class="mb-3">
                            <form action="<?= BASE_URL ?>orders.php" method="get" class="row g-3">
                                <div class="col-md-5">
                                    <div class="input-group">
                                        ...
                                        <a href="<?= BASE_URL ?>orders.php<?= isset($_GET['department_id']) && !empty($_GET['department_id']) ? '?department_id=' . $_GET['department_id'] : '' ?>" class="btn btn-outline-danger">
                        ...
                        <?php if (empty($documents)): ?>
                            <div class="alert alert-info">
                                ...
                                <?php if (!empty($title_filter) || $department_filter > 0): ?>
                                    Накази за заданими критеріями не знайдені.
                                <?php else: ?>
                                    У системі ще немає наказів.
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="document-icon me-3">
                                                    <i class="bi <?= $file_icon ?> text-success fs-1"></i>
                                                </div>
                                                ...
                                                <div class="document-actions">
                                                    <div class="btn-group">
                                                        ...
                                                        <a href="<?= BASE_URL ?>uploads/<?= $doc['file_path'] ?>" download="<?= htmlspecialchars($doc['title']) ?>.<?= strtolower(pathinfo($doc['file_path'], PATHINFO_EXTENSION)) ?>" class="btn btn-sm btn-outline-danger" title="Завантажити">
                                ...
                                <nav aria-label="Навігація по сторінках">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                                            <a class="page-link" href="<?= BASE_URL ?>orders.php?page=<?= $page - 1 ?><?= !empty($title_filter) ? '&title=' . urlencode($title_filter) : '' ?><?= $department_filter > 0 ? '&department_id=' . $department_filter : '' ?>">
                                                <i class="bi bi-chevron-left"></i>
                                            </a>
                                        </li>
                                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                            <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                                                <a class="page-link" href="<?= BASE_URL ?>orders.php?page=<?= $i ?><?= !empty($title_filter) ? '&title=' . urlencode($title_filter) : '' ?><?= $department_filter > 0 ? '&department_id=' . $department_filter : '' ?>">
                                                    <?= $i ?>
                                                </a>
                                            </li>
                                        <?php endfor; ?>
        ...
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title" id="viewDocumentModalLabel">
                    <i class="bi bi-journal-text me-2"></i> Перегляд наказу
                </h5>
            ...
            <div class="modal-body p-0">
                <div id="documentViewerContainer" class="document-viewer-container">
                    ...
                    <div class="text-center p-5" id="documentLoading">
                        ...
                        <p class="mt-3">Завантаження наказу...</p>
                    </div>
